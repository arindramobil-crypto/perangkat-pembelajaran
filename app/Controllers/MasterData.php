...
class MasterData extends BaseController
{
    ...
    public function toggle_status_tahun($id)
    {
        $model  = new TahunPelajaranModel();
        $tahun  = $model->find($id);

        if (! $tahun) {
            return redirect()->to('/master/tahun-pelajaran')->with('error', 'Data tidak ditemukan.');
        }
        ...
        if ($tahun['status'] === 'Tidak Aktif') {
            ...
            $model->update($id, ['status' => 'Tidak Aktif']);
            $msg = "Tahun pelajaran {$tahun['tahun']} ({$tahun['semester']}) dinonaktifkan.";
        }

        return redirect()->to('/master/tahun-pelajaran')->with('success', $msg);
    }
