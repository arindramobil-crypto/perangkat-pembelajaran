...
<div class="lms-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1" style="color: var(--lms-text);">
                <i class="bi bi-calendar3 me-2" style="color: var(--lms-primary);"></i>Data Tahun Pelajaran
            </h4>
            <p class="mb-0 small" style="color: var(--lms-text-muted);">Kelola tahun pelajaran dan periode aktif</p>
        </div>
    ...
    <div style="overflow-x: auto;">
        <table class="datatable" id="tahunTable" style="width: 100%;">
            <thead>
                <tr>
                    <th>No</th><th>Tahun</th><th>Semester</th><th>Status</th><th>Aksi</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td><?= $no++ ?></td>
                    <td><strong><?= esc($t['tahun']) ?></strong></td>
                    <td>
                        <?php $semColor = $t['semester'] === 'Ganjil' ? '#6366F1' : '#EC4899'; ?>
                        <span style="display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:20px;font-size:.78rem;font-weight:600;background:<?= $t['semester']==='Ganjil'?'rgba(99,102,241,.12)':'rgba(236,72,153,.12)' ?>;color:<?= $semColor ?>;">
                            <i class="bi <?= $t['semester']==='Ganjil'?'bi-1-circle':'bi-2-circle' ?>"></i>
                            Semester <?= esc($t['semester']) ?>
                        </span>
                    </td>
                    <td>
                        <?php if ($t['status'] === 'Aktif'): ?>
                            <a href="<?= base_url('master/tahun-pelajaran/toggle-status/'.$t['id']) ?>" 
                               style="display:inline-flex;align-items:center;gap:4px;padding:3px 12px;border-radius:20px;font-size:.78rem;font-weight:700;background:rgba(16,185,129,.12);color:#10B981;border:1px solid rgba(16,185,129,.25);text-decoration:none;transition:all .2s;" 
                               onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 2px 5px rgba(16,185,129,.2)';" 
                        ...
                        <?php else: ?>
                            <a href="<?= base_url('master/tahun-pelajaran/toggle-status/'.$t['id']) ?>" 
                               style="display:inline-flex;align-items:center;gap:4px;padding:3px 12px;border-radius:20px;font-size:.78rem;font-weight:600;background:var(--lms-hover-bg);color:var(--lms-text-muted);border:1px solid var(--lms-border);text-decoration:none;transition:all .2s;" 
                               onmouseover="this.style.background='rgba(16,185,129,.1)';this.style.color='#10B981';this.style.borderColor='rgba(16,185,129,.25)';this.style.transform='translateY(-1px)';" 
                               onmouseout="this.style.background='var(--lms-hover-bg)';this.style.color='var(--lms-text-muted)';this.style.borderColor='var(--lms-border)';this.style.transform='';" 
                    ...
                    <td>
                        <a href="<?= base_url('master/tahun-pelajaran/delete/'.$t['id']) ?>"
                           class="lms-btn-danger-sm"
                           onclick="return confirmDelete(this, '<?= esc($t['tahun']) ?> - <?= esc($t['semester']) ?>')">
...
<!-- MODAL TAMBAH TAHUN -->
<div id="tahunModal" class="lms-modal-overlay" onclick="if(event.target===this)hideModal()">
    <div class="lms-modal-box" id="tahunModalBox">
        <div class="lms-modal-header">
            <div class="lms-modal-icon"><i class="bi bi-calendar-plus"></i></div>
            <div>
                <h5 class="lms-modal-title">Tambah Tahun Pelajaran</h5>
                <p class="lms-modal-subtitle">Isi informasi periode tahun pelajaran baru</p>
            </div>
            <button class="lms-modal-close" onclick="hideModal()"><i class="bi bi-x-lg"></i></button>
        </div>
        <form action="<?= base_url('master/tahun-pelajaran') ?>" method="post" class="lms-modal-body">
            <?= csrf_field() ?>
            <input type="hidden" name="id" id="tahun_id">
            <div class="lms-form-group">
                <label class="lms-form-label"><i class="bi bi-calendar3 me-1"></i>Tahun Pelajaran</label>
                <input type="text" name="tahun" id="tahun" class="lms-form-input"
                       placeholder="Contoh: 2025/2026" required>
            </div>
            <div class="lms-form-row">
                <div class="lms-form-group">
                    <label class="lms-form-label"><i class="bi bi-layers me-1"></i>Semester</label>
                    <select name="semester" id="semester" class="lms-form-select" required>
                        <option value="Ganjil">Ganjil</option>
                        <option value="Genap">Genap</option>
                    </select>
                </div>
                <div class="lms-form-group">
                    <label class="lms-form-label"><i class="bi bi-toggle-on me-1"></i>Status</label>
                    <select name="status" id="status" class="lms-form-select" required>
            ...
            <div class="lms-modal-footer">
                ...
                <button type="submit" class="lms-btn-submit"><i class="bi bi-check2-circle me-1"></i>Simpan</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="lms-modal-box lms-modal-sm" id="deleteModalBox">
        ...
        <div class="lms-modal-body">
            <p style="color:var(--lms-text);margin-bottom:0;">
                Yakin hapus tahun pelajaran <strong id="deleteTargetName" style="color:var(--lms-danger);">—</strong>?
            </p>
        </div>
        <div class="lms-modal-footer">
...
<style>
.lms-form-row { display:grid; grid-template-columns:1fr 1fr; gap:14px; }
.lms-btn-primary { background:linear-gradient(135deg,var(--lms-primary),var(--lms-accent)); border:none; color:#fff; font-weight:600; padding:9px 18px; border-radius:10px; box-shadow:0 4px 12px rgba(37,99,235,.3); transition:all .2s; cursor:pointer; display:inline-flex; align-items:center; gap:6px; }
...
<script>
function showModal() {
    document.getElementById('tahunModal').classList.add('active');
    setTimeout(() => document.getElementById('tahun').focus(), 100);
    document.body.style.overflow = 'hidden';
}
function hideModal() {
    const box = document.getElementById('tahunModalBox');
    box.style.animation = 'modalSlideOut .2s ease forwards';
    setTimeout(() => { document.getElementById('tahunModal').classList.remove('active'); box.style.animation = ''; document.body.style.overflow = ''; }, 190);
}
...
document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
        if (document.getElementById('tahunModal').classList.contains('active')) hideModal();
